...
pub fn run(run_info_path: &str) {
    ...
    let run_info: RunInfo =
        serde_json::from_str(&run_info_str).expect("Failed to parse run info data");

    println!(
        "Executing process '{}' in directory '{}'",
        run_info.program, run_info.work_dir
    );
    ...
    while window_handles.is_none() {
        window_handles = platform::get_process_windows(pid, run_info.process_all_windows);
    }
    ...
    thread::scope(|s| {
        let handle = s.spawn(|| {
            ...
            println!(
                ...
                &run_info.program, output.status
            );
        });
        ...
        for event in &run_info.events {
            ...
            platform::process_event(&window_handles, event);
        }
